...
function delete_employer(id){
	var myurl = baseUrl+'admin/participants/delete_employer/'+id;
	var is_confirm = confirm("Are you sure you want to delete this Participants ?");
	if(is_confirm){
		  $.get(myurl, function (sts){
			  if(sts=='done')
				  $("#row_"+id).fadeOut();
...
// ==============Start Ranks Module============
function load_rankes_edit_form(id){
	
	$.getJSON(baseUrl+'admin/rankes/get_rank_by_id/'+id, function(data) {
			$('#edit_rank_name').val(data.rank_name);
			$('#edit_rank_position').val(data.rank_position);
			$('#rankes_id').val(data.ID);
			$('#edit_page_form').modal('show');
		});	
		
}
function delete_rankes(id){
	var myurl = baseUrl+'admin/rankes/delete/'+id;
	var is_confirm = confirm("Are you sure you want to delete this rank?");
	if(is_confirm){
		  $.get(myurl, function (show) {
			  if(show=='done')
				  $("#row_"+id).fadeOut();
...
// ==============Start Ships Module============
function load_ships_edit_form(id){
	
	$.getJSON(baseUrl+'admin/ships/get_ship_by_id/'+id, function(data) {
			$('#edit_ship_name').val(data.ship_name);
			$('#ships_id').val(data.ID);
			$('#edit_page_form').modal('show');
		});	
		
}
function delete_ships(id){
	var myurl = baseUrl+'admin/ships/delete/'+id;
	var is_confirm = confirm("Are you sure you want to delete this ship?");
	if(is_confirm){
		  $.get(myurl, function (show) {
			  if(show=='done')
				  $("#row_"+id).fadeOut();
